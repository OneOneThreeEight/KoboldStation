	...
	switch(find_type)
		...
		if(ARCHAEO_CUTLERY)
			...
			if(prob(25))
				...
				new_item = new /obj/item/material/kitchen/utensil/knife(src.loc)
			else
		...
		if(ARCHAEO_BEARTRAP)
			...
			apply_prefix = 0
			new_item = new /obj/item/trap(src.loc)
		...
		if(ARCHAEO_STOCKPARTS)
			var/list/possible_spawns = subtypesof(/obj/item/stock_parts) + subtypesof(/obj/item/circuitboard)
