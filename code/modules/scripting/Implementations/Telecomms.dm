...
/datum/TCS_Compiler
	...
	proc/Run(var/datum/signal/signal)
		...
		//Language macros
		interpreter.SetVar("L_KOBOLD",	LANGUAGE_KOBOLD)
		interpreter.SetVar("L_DRACONIC",	    LANGUAGE_DRAGON)
...
datum/signal
	...
	proc/tcombroadcast(var/message, var/freq, var/source, var/job, var/verb, var/language)
		...
		if(!L || !(L.flags & TCOMSSIM))
			L = all_languages[LANGUAGE_KOBOLD]
		...
		var/tmp/datum/radio_frequency/connection = SSradio.return_frequency(freq)
